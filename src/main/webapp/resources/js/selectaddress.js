...
function populateCountriesList(countryElementId, cIndex) {
    var countryElement = document.getElementById(countryElementId);
    countryElement.length = 0;
    countryElement.options[0] = new Option('Select Country', '-1');
    countryElement.selectedIndex = 0;
    ...
    countryElement.selectedIndex = cIndex;
}

function populateStatesList(stateElementId, selectedCountryIndex, stateIndex) {
    var stateElement = document.getElementById(stateElementId);
    stateElement.length = 0;
    stateElement.options[0] = new Option('Select State', '');
    stateElement.selectedIndex = 0;
...
function populateStatesBasedOnCountry(stateElementId, selectedCountryIndex) {
    var stateElement = document.getElementById(stateElementId);
    stateElement.length = 0;
    stateElement.options[0] = new Option('Select State', '');
    stateElement.selectedIndex = 0;
...
function getCountryIndex(countryValue){	
	var countryIndex = 0;
	for (var i = 0; i < country_arr.length; i++) {
        if(country_arr[i] == countryValue){
        	countryIndex = i+1;
        }
    }
	return countryIndex;
...
function getStateIndex(stateValue, countryIndex){	
	var stateIndex = 0;
	var state_arr = s_a[countryIndex].split("|");	
	for (var i = 0; i < state_arr.length; i++) {
        if(state_arr[i] == stateValue){
        	stateIndex = i+1;
        }
    }
	return stateIndex;
...
function populateCountryStateSelection(countryValue, stateValue, countryElementId, stateElementId){	
	var cIndex = this.getCountryIndex(countryValue);
	var sIndex = this.getStateIndex(stateValue, cIndex);
	populateCountries(countryElementId, stateElementId);
	populateCountriesList(countryElementId, cIndex);			  
    populateStatesList(stateElementId, cIndex, sIndex);
}

function populateCountrySelection(countryValue, countryElementId, stateElementId){	
	var cIndex = this.getCountryIndex(countryValue);
	populateCountriesList(countryElementId, cIndex);			  
	populateStatesBasedOnCountry(stateElementId, cIndex);
}
